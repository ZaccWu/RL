...
class dqnModel3:
  ...
  def QNetworkStructure(self):
    ...
    with tf.variable_scope('current_net'):
        ...
        W3e = self.WeightVariable([hWidth,hWidth])
        b3e = self.BiasVariable([hWidth])
        W4e = self.WeightVariable([hWidth, self.actionDim])
        b4e = self.BiasVariable([self.actionDim])
        # hidden layers
        hiddenLayer1 = tf.nn.relu(tf.matmul(self.stateInput, W1e) + b1e)
        hiddenLayer2 = tf.nn.relu(tf.matmul(hiddenLayer1, W2e) + b2e)
        hiddenLayer3 = tf.nn.relu(tf.matmul(hiddenLayer2, W3e) + b3e)
        # Q Value layer
        self.QValue = tf.matmul(hiddenLayer3, W4e) + b4e
    ...
    with tf.variable_scope('target_net'):
        ...
        W3t = self.WeightVariable([hWidth,hWidth])
        b3t = self.BiasVariable([hWidth])
        W4t = self.WeightVariable([hWidth, self.actionDim])
        b4t = self.BiasVariable([self.actionDim])
        # hidden layers
        ...
        hiddenLayerTarget3 = tf.nn.relu(tf.matmul(hiddenLayerTarget2, W3t) + b3t)
        # Q Value layer
        self.targetQValue = tf.matmul(hiddenLayerTarget3, W4t) + b4t
