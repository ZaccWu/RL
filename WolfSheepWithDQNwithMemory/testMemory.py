...
def trainTask(dqn,actionSpace):
    ...
    trainPlot = 1000 # plot while training
    testPlot = 15 # plot while testing
    ...
    for episode in range(EPISODE):
        ...
        # memory part
        memoryState = np.array([])
        stateDim = len(flatten(state))
        for i in range(5):
            memoryState=np.hstack((memoryState,flatten(state)))

        for t in range(STEP):
            actionId = dqn.egreedyAction(memoryState)
            nextState = transitFunction(state, actionSpace[actionId])
            nextMemoryState = np.hstack((memoryState[stateDim:],flatten(nextState)))    # add memory
            ...
            dqn.Update(memoryState, actionId, reward, nextMemoryState, done)
            state = nextState
            memoryState = nextMemoryState   # add memory
            meanRewards += 1
            if done:
                break
        # plot when training
        ...
        if episode == EPISODE-1:
            for people in range(10):
                ...
                for k in [0, 1, 2, 3, 4, 5, 6]:
                    ...
                    for i in range(testPlot):
                        ...
                        for j in range(STEP):
                            ...
                            actionId = dqn.GetMaxAction(memoryState)    # add memory
                            nextState = transitFunction(state, actionSpace[actionId])
                            nextMemoryState = np.hstack((memoryState[stateDim:], flatten(nextState)))  # add memory
                            done = isTerminal(nextState)
                            reward = rewardFunction(state, actionSpace[actionId], nextState)
                ...
                print("# peopleIdx:{},performance:{}".format(people, performance))
            # print("p =",results)
...
def main():
    ...
    stateDim = 500       # watch out this! 20*NumofAgent
