...
class dqnModel:
  ...
  def QNetworkStructure(self):
    ...
    with tf.variable_scope('current_net'):
        ...
        W2e = self.WeightVariable([hWidth, self.actionDim])
        b2e = self.BiasVariable([self.actionDim])
        # hidden layers
        hiddenLayer = tf.nn.relu(tf.matmul(self.stateInput, W1e) + b1e)
        # Q Value layer
        self.QValue = tf.matmul(hiddenLayer, W2e) + b2e
